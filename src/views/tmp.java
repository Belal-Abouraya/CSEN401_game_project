...
public class tmp {
	...
	private static double RectangleWidth = 100 ;
	private static double RectangleHeight = 100 ;
	...
	public Scene getScene() {
		...
		// add it to the sceneBack
		sceneBack.getChildren().add(wallpaper);
		// right -> gridPane , left -> vbox (imageView , labels)
		createHeroes();
		// create the scene 
		vbox = createModel(map[0][0]);
		vbox.setAlignment(Pos.CENTER_LEFT);
		heroes.setAlignment(Pos.BOTTOM_CENTER);
		heroes.setTranslateY(-2);
		heroes.setHgap(2);
		heroes.setVgap(2);
		Label selectYourHero = createSelectYourHeroLabel();
//		selectYourHero.setAlignment(Pos.BASELINE_CENTER);
		sceneBack.getChildren().addAll(vbox , heroes , selectYourHero);
...
//			borderPane.setLeft(createModel(map[row][column]));
			sceneBack.getChildren().clear();
			vbox = createModel(map[row][column]);
			vbox.setAlignment(Pos.CENTER_LEFT);
			sceneBack.getChildren().addAll(wallpaper , vbox , heroes , selectYourHero);
		});
		return scene ;
	}
	...
	private static VBox createModel(Hero h) {
		...
		Label info = new Label(
				...
				h.getType() + "\n" +
				"Health : "+ h.getMaxHp() + "\n" +
				"Number of Actions per Turn : "+h.getMaxActions() + "\n" + 
	...
	private static Label createSelectYourHeroLabel() {
		Label selectYourHero = new Label("Select Your Hero");
