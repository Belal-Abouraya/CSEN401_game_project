		...
		sceneBack.setOnKeyPressed(e -> {
			...
			if (isValid) {
				...
				case ENTER -> {
					...
					if (res == 0) {
						Game.mode = "classic";
						Main.window.getScene().getStylesheets()
								.add(this.getClass().getResource("classic.css").toExternalForm());
						Main.window.getScene().setRoot((new SecondScene()).getRoot());
					} else {
						Game.mode = "bonus";
						Main.window.getScene().getStylesheets()
								.add(this.getClass().getResource("bonus.css").toExternalForm());
						// Main.window.getScene().setRoot((new SecondScene()).getRoot());
					}
				}
				}
...
//		res.setOnMouseExited(e -> hover.stop());
		res.setOnMouseClicked(e -> {
			...
			if (count == 0) {
				...
				Game.mode = "bonus";
				Main.window.getScene().getStylesheets().add(this.getClass().getResource("bonus.css").toExternalForm());
				// Main.window.getScene().setRoot((new SecondScene()).getRoot());
			}
		});
